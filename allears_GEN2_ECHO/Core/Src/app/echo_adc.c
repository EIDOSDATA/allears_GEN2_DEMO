...
uint32_t Echo_ADC_Calc_Stepup_V(uint32_t in_adc_val, uint32_t r1, uint32_t r2)
{
	...
	/* REFERENCE TABLE */
	if (ECHO_VOLTAGE_RELATED_PULSE_WIDTH > 24)
	{
		ECHO_VOLTAGE_RELATED_PULSE_WIDTH = 24;
	}
	uint64_t adc_val = (ECHO_REF_ADC_VOLTAGE_TABLE[ECHO_VOLTAGE_RELATED_PULSE_WIDTH]
			* in_adc_val)
			/ ECHO_REF_ADC_VALUE_TABLE[ECHO_VOLTAGE_RELATED_PULSE_WIDTH];
	uint64_t v_out = (ECHO_REF_VOLTAGE_TABLE[ECHO_VOLTAGE_RELATED_PULSE_WIDTH]
			* adc_val)
			/ ECHO_REF_ADC_VOLTAGE_TABLE[ECHO_VOLTAGE_RELATED_PULSE_WIDTH];
